...
function App() {
  ...
  const categoryOptions = useMemo(() => Array.from(new Set(parts.map((part) => part.category).filter(Boolean))).sort(), [parts]);
  const filteredInventoryParts = useMemo(() => parts.filter((part) => (!categoryFilter || part.category === categoryFilter) && (!locationFilter || part.location === locationFilter)), [categoryFilter, locationFilter, parts]);
  ...
  return (
    <main className="app">
      ...
      {activeMainTab === 'inventory' && (
        <div role="tabpanel">
          <section className="toolbar card"><input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void loadParts()} placeholder="SKU, 품명, 제조사, 위치 검색" /><select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">전체 분류</option>{categoryOptions.map((category) => <option key={category} value={category}>{category}</option>)}</select><select value={locationFilter} onChange={(event) => setLocationFilter(event.target.value)}><option value="">전체 위치</option>{locations.map((loc) => <option key={loc.id} value={loc.code}>{loc.code}</option>)}</select><button onClick={() => void loadParts()}>{loading ? '검색 중...' : '검색'}</button><a className="button-link" href="/api/parts/export.csv">CSV export</a><label className="file-button">CSV import<input type="file" accept=".csv,text/csv" onChange={(e) => void importCsv(e.target.files?.[0])} /></label><label className="check"><input type="checkbox" checked={lowStockOnly} onChange={(event) => setLowStockOnly(event.target.checked)} /> 부족 재고만</label></section>
          <div className="grid">
            <section className="card table-card"><h2>재고 목록</h2><div className="table-wrap"><table><thead><tr><th className="hint" data-tooltip="Stock Keeping Unit: 재고 품목을 구분하는 고유 관리 코드입니다.">SKU</th><th>품명</th><th>분류</th><th>위치</th><th>수량</th><th></th></tr></thead><tbody>{filteredInventoryParts.map((part) => <tr key={part.id} className={part.quantity <= part.minQuantity ? 'low' : ''} onClick={() => setSelectedId(part.id)}><td title={part.sku}>{part.sku}</td><td><b>{part.name}</b><small>{part.manufacturer} {part.mpn}</small></td><td>{part.category}</td><td>{part.location || '-'}</td><td>{part.quantity} {part.unit}<small>min {part.minQuantity}</small></td><td><button onClick={(event) => { event.stopPropagation(); edit(part); }}>수정</button></td></tr>)}</tbody></table></div></section>
